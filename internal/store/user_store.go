...
func (s *PostgresUserStore) UpdateUser(user *User) error {
	query := `UPDATE users SET username = $1, email = $2, bio = $3,
		updated_at = CURENT_TIMESTAMP WHERE id = $4 RETURNING updated_at`

	res, err := s.db.Exec(query, user.Username, user.Email, user.Bio, user.ID)
	if err != nil {
		return err
	}
...
func (s *PostgresUserStore) GetUserToken(scope, plaintextPassword string) (*User, error) {
	tokenHash := sha256.Sum224([]byte(plaintextPassword))
